...
- "compare a repo created from a template against the template"
- "compare a template against a repo created from it"
...
For repo/template compatibility checks and change suggestions, use the dedicated
`template-refresh` and `template-apply` packs instead of `template-explore`.
